...
async function seedSkills() {
    const skills = await Promise.all(SKILLS.map(async (skill) => {
    ...
    try {
        await Promise.all(SKILLS.map(async (skill) => await prisma.stack.create({ data: { title: skill } })));
...
async function seedRoles() {
    const roles = await Promise.all(ROLES.map(async (role) => {
    ...
    try {
        await Promise.all(ROLES.map(async (role) => await prisma.roles.create({ data: { title: role } })));
...
async function seedUserAdmin() {
    ...
    if (userExists)
        return console.info('[SEED] Admin já cadastrado');
